...
fn main() -> Result<()> {
    ...
    let analyzer = Analyzer::try_new(content.into(), cli.base, 0, kaiko::cpu_mode::CpuMode::Arm)?;
    ...
    match cli.mode {
        Mode::Preloader => {
            ...
            let bldr_jump = BldrJump::new(&analyzer)
                .extract()
                .context("error on extracting bldr_jump ptr")?;
            println!("bldr_jump: {bldr_jump:#x}");
        }
        Mode::LK => {
